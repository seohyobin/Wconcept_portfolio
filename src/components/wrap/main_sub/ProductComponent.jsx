...
export default function ProductComponent(){
    ...
    const [amount,setCnt]=React.useState(1);

    React.useEffect(()=>{

        if(product1.disPrice===''){
            setState({
                ...state,
                product1:{
                    ...state.product1,
                    amount:amount,
                    allPrice: amount*product1.basePrice
            ...
            setState({
                ...
                product1:{
                    ...state.product1,
                    amount:amount,
                    allPrice: amount*product1.disPrice
            ...
            if(product1.disPrice===''){
                setState({
                    ...
                    product1:{
                        ...state.product1,
                        amount:1,
                        allPrice:Math.round(amount*(product1.basePrice))
                ...
                setState({
                    ...
                    product1:{
                        ...state.product1,
                        amount:1,
                        allPrice:Math.round(amount*(product1.disPrice))
    ...
    React.useEffect(()=>{
        ...
        if(isCartOk===true){ 
            ...
            if(localStorage.getItem('CARTPRODUCT')===null){
                ...
                arr = arr.map((item)=>item.id === product1.id ? {...item,amount:item.amount+=product1.amount , allPrice:item.allPrice+=product1.allPrice} : item);
                const result = arr.map((item)=>item.id===product1.id ? true : false);

                if(result.includes(true) !== true){
                    arr = [product1, ...arr]
                    console.log(`장바구니배열`,arr);   
                }
            } 
                        ...
                        <div className="row1">
                            <ul className='row1-ul'>
                                ...
                                <li className='row1-li2'>
                                    ...
                                    <span className='reserve' style={(product1.reserve==='' ? {display:'none',  marginRight:'-50px', height:'0px', padding:'0px'} : {})}>
                                        {product1.reserve}
                                    </span>
                                </li>
                                    ...
                                    <dl>
...

                                        {
                                         ...
                                         <>
                                            ...
                                            <em className='em2' style={(Number(product1.disPrice)=== 0 ? {display:'none'}: {})}>{Number(product1.disPrice).toLocaleString('ko-KR')}</em>
                                         </>
                                         )
                                         :
                                         (
                            ...
                            <div class="card_info_wrap">
                                <div className="box3">
                                    ...
                                    <div className='dd3'>
                                        ...
                                        (<span><em>{product1.basePrice!== '' ? Number(amount*(Math.round((1-0.9)*product1.basePrice))).toLocaleString('ko-KR') :Number(Math.floor((1-0.9)*product1.disPrice)).toLocaleString('ko-KR')}</em> p 적립&nbsp;</span>)
                                        :
                                        (<span><em>{product1.disPrice!== '' ? Number(amount*(Math.round((1-0.9)*product1.disPrice))).toLocaleString('ko-KR') :Number(Math.floor((1-0.9)*product1.basePrice)).toLocaleString('ko-KR')}</em> p 적립&nbsp;</span>)
                            ...
                            <div className="row4-size">
                                ...
                                <div className="allall">
                                    ...
                                    <div className="all-price">
                                        ...
                                        (<span>{product1.basePrice!== '' ? Number(amount*(product1.basePrice)).toLocaleString('ko-KR') : Number(amount*(product1.disPrice)).toLocaleString('ko-KR')} <span className='won'>원</span></span>)
                                       :
                                        (<span>{product1.disPrice!== '' ? Number(amount*(product1.disPrice)).toLocaleString('ko-KR') : Number(amount*(product1.basePrice)).toLocaleString('ko-KR')} <span className='won'>원</span></span>)
                                        }
                                    </div>
                                </div>
                            </div>
